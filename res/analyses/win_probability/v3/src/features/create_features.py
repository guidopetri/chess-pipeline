...
df['half_move_sq'] = df['half_move'] ** 2
df['half_move_inv'] = 1 / df['half_move']
df['half_move_inv_sq'] = df['half_move_inv'] ** 2

df['result_mapped'] = df['result'].map({'1-0': 1, '0-1': 0})
df['winner_clock'] = (df['result_mapped'] * df['white_sig_clock_pct']
                      + (1 - df['result_mapped']) * df['black_sig_clock_pct'])

pace_of_play = sm.ols('winner_clock ~ half_move + half_move_sq '
                      '+ half_move_inv + half_move_inv_sq',
                      data=df[df['result'] != '1/2-1/2'])
pace_of_play = pace_of_play.fit()
print(pace_of_play.summary())

df['winner_pace_of_play'] = pace_of_play.predict(df)
...
pace_of_play.save('../../models/pace_of_play.pckl')
